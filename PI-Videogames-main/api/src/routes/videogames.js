...
const { getApiInfo, getInfoTotal, postGame } = require("./controllers")
const {
    API_KEY
} = process.env;
...
router.get("/:id", async (req, res) => {
    const { id } = req.params
    try {
        const urlInfo = await axios.get(`https://api.rawg.io/api/games/${id}?key=${API_KEY}`).then(el=>el.data)
        console.log(urlInfo)
        if (urlInfo=== undefined) {
            return res.status(404).send("not found")
        } else { return res.status(200).send(urlInfo) }
